...
//			@RequestParam("userName") String userName
		...
		for(int i = dto.getUserName().length(); dto.getUserName().length()<6;i++) {
			dto.setUserName("0"+dto.getUserName());
		}
	...
	public ModelAndView detail(
			@RequestParam(required = false, value = "boardId") Integer id
			) {
		ModelAndView mav = new ModelAndView();
		try {
...
//	}
	...
	@RequestMapping(value="/board/update", method=RequestMethod.POST)
	public ModelAndView postUpdate(
			@ModelAttribute BoardDTO modelDTO
//			@RequestParam("boardId") Integer id
			) {
		logger.info(modelDTO.toString());
		ModelAndView mav = new ModelAndView();
		if(boardService.updateById(modelDTO) == 0) {
			mav.setViewName("redirect:/board/update");
		}else {
			mav.setViewName("redirect:/board/detail");
		}
		mav.addObject("boardId",modelDTO.getId());
		return mav;
	}
